...
const projDir = fs.mkdirSync(path.join(tmp, 'e2e-project'), { recursive: true }) ?? path.join(tmp, 'e2e-project');
...
const bail = async (msg) => { console.error(`ABORT - ${msg}`); await cleanup(); process.exit(1); };

let PORT = 0, TOKEN = '', child = null;
const api = (p, opts = {}) => fetch(`http://127.0.0.1:${PORT}/api${p}`, {
  ...opts,
...
const stopServer = () => new Promise((res) => { child.once('exit', res); child.kill(); });
async function cleanup() {
  try { await stopServer(); } catch { /* already down */ }
  await sleep(1500); // node-pty children can hold cwd for a beat (EBUSY)
  try { fs.rmSync(tmp, { recursive: true, force: true }); } catch { /* temp — OS cleans it */ }
}
...
// The drift check must see a real, current claude.
{
  ...
  while (!(d = await (await api('/diagnostics')).json()).claude.checked && Date.now() < deadline) await sleep(300);
  check('real claude CLI present + at/above tested floor', d.claude.ok === true,
    `version=${d.claude.version}`);
}

await api('/workspaces', { method: 'POST', body: JSON.stringify({ name: 'e2e', dir: projDir }) });
const created = await (await api('/sessions', { method: 'POST', body: JSON.stringify({ workspace: projDir, cols: 120, rows: 30 }) })).json();
const id = created.id;
...
check('SessionStart hook → pane became ready (activity set)', Boolean(s?.activity), `activity=${s?.activity}`);
if (!s?.activity) { await bail('claude never reported a session — likely stuck at a prompt'); }
...
check('UserPromptSubmit hook → badge "working"', s?.activity === 'working', `activity=${s?.activity}`);

s = await waitFor(id, (x) => x.activity === 'idle', 120000, 'Stop → idle');
check('Stop hook → badge "idle" (turn finished)', s?.activity === 'idle', `activity=${s?.activity}`);

// -------------------------------------- phase 4: transcript, usage, title
// The public API exposes hasTranscript/canResume/summary (not the raw path/id).
check('transcript captured + resumable (hasTranscript && canResume)',
  s?.hasTranscript === true && s?.canResume === true,
  `hasTranscript=${s?.hasTranscript} canResume=${s?.canResume}`);
check('pane auto-title = the opening prompt', s?.summary === PROMPT, `summary=${JSON.stringify(s?.summary)}`);

{
  ...
  const totalIn = Object.values(u.models ?? {}).reduce((n, m) => n + m.input + m.cacheRead + m.cacheWrite, 0);
  const totalOut = Object.values(u.models ?? {}).reduce((n, m) => n + m.output, 0);
  check('real usage parsed from transcript (tokens > 0)', u.available && totalIn > 0 && totalOut > 0,
...
s = await sessionInfo(id);
check('after restart: pane persisted as dead + resumable', s?.status === 'dead' && s?.canResume === true,
  `status=${s?.status} canResume=${s?.canResume}`);

await api(`/sessions/${id}/revive`, { method: 'POST', body: JSON.stringify({ cols: 120, rows: 30 }) });
s = await waitFor(id, (x) => x.status === 'running' && x.hasTranscript, 90000, 'revive back to running');
...
// public fields.
check('revive resumed the SAME conversation (opening prompt preserved)',
