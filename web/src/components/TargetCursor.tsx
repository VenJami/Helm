...
// Animated custom cursor (from React Bits, ported to TS for Helm): an amber
// dot with four spinning corner brackets that lock onto interactive elements.
...
interface TargetCursorProps {
  ...
  /** Only show the custom cursor while the mouse is inside this element;
      outside it the native cursor stays untouched. */
...
export function TargetCursor({
  ...
  useEffect(() => {
    ...
    // Class (not body.style.cursor) so it beats per-element cursor styles
    // like .btn { cursor: pointer } — see TargetCursor.css.
    if (hideDefaultCursor) {
      document.body.classList.add('target-cursor-hide');
    }

    const cursor = cursorRef.current;
    cornersRef.current = cursor.querySelectorAll<HTMLDivElement>('.target-cursor-corner');

    // When scoped, the cursor starts hidden and only fades in inside the scope.
    // Resolve the scope element LIVE (not once at mount) so it survives the
    // scoped container being unmounted/remounted — e.g. hiding then re-showing
    // the sidebar. A missing scope element means "not inside" (cursor stays
    // hidden), never "global".
    const getScopeEl = () =>
      scopeSelector ? document.querySelector<HTMLElement>(scopeSelector) : null;
    let insideScope = !scopeSelector;
    ...
    const moveHandler = (e: MouseEvent) => {
      if (scopeSelector) {
        const scopeEl = getScopeEl();
        const inside = !!scopeEl && e.target instanceof Node && scopeEl.contains(e.target);
        if (inside !== insideScope) {
          insideScope = inside;
          gsap.to(cursor, { autoAlpha: inside ? 1 : 0, duration: 0.15, overwrite: 'auto' });
        }
        if (!inside) return; // don't track moves outside the scope
      }
    ...
    const mouseDownHandler = () => {
      if (!dotRef.current || !insideScope) return;
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      if (tickerFnRef.current) {
        // Remove before add so a target unmounted mid-hover (e.g. sidebar
        // hidden) can't leave the ticker fn stacked/running twice.
    ...
    return () => {
      ...
      spinTl.current?.kill();
      document.body.classList.remove('target-cursor-hide');
