...
type PaletteItem =
  ...
  | { kind: 'action'; key: string; label: string; icon: 'plus' | 'broadcast' | 'chart'; run: () => void };
...
const paneStatus = (s: SessionInfo) =>
  s.status === 'running' ? s.activity ?? 'live' : s.status;

export function CommandPalette({
  onClose, sessions, workspaces, onJumpToPane, onSelectWorkspace, onNewPane, onBroadcast, onUsage,
}: Props) {
  ...
  const items = useMemo<PaletteItem[]>(() => {
    ...
    const paneItems: PaletteItem[] = sessions
      .map((s) => ({ kind: 'pane' as const, key: `pane-${s.id}`, session: s, ws: wsName(s.workspace) }))
      .filter((it) =>
        !q || it.session.name.toLowerCase().includes(q) || it.ws.toLowerCase().includes(q)
        || (it.session.summary?.toLowerCase().includes(q) ?? false));

    const wsItems: PaletteItem[] = workspaces
      .map((w) => ({ kind: 'workspace' as const, key: `ws-${w.id}`, id: w.id, name: w.name, dir: w.dir }))
    ...
    if (onNewPane) actionDefs.push({ kind: 'action', key: 'act-new', label: 'New pane', icon: 'plus', run: onNewPane });
    if (onBroadcast) actionDefs.push({ kind: 'action', key: 'act-bc', label: 'Broadcast to panes', icon: 'broadcast', run: onBroadcast });
    if (onUsage) actionDefs.push({ kind: 'action', key: 'act-usage', label: 'Usage by account', icon: 'chart', run: onUsage });
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    else if (e.key === 'Enter') { e.preventDefault(); activate(items[Math.min(active, items.length - 1)]); }
  };
              ...
              >
                {it.kind === 'pane' && (
                  <>
                    ...
                    <span className="cmdk-name" style={{ color: it.session.color }}>{it.session.name}</span>
                    <span className="cmdk-summary">{it.session.summary ?? <em>no prompt yet</em>}</span>
                ...
                {it.kind === 'workspace' && (
                  <>
                    <span className="cmdk-glyph"><IconFolder size={13} /></span>
                    <span className="cmdk-name">{it.name}</span>
                    <span className="cmdk-where" title={it.dir}>{it.dir}</span>
                  </>
                )}
                {it.kind === 'action' && (
                  <>
